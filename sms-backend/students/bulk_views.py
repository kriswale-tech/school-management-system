...
from students.bulk_import import (
    commit_student_bulk_import,
    detect_upload_format,
    load_failure_export,
    parse_student_bulk_upload,
    preview_student_bulk_import,
)
from students.bulk_serializers import (
    StudentBulkImportConfirmResponseSerializer,
    StudentBulkImportPreviewResponseSerializer,
)
from students.bulk_template import build_student_import_template
...
def _validate_uploaded_file(uploaded_file):
    ...
    if uploaded_file.size > settings.STUDENT_BULK_IMPORT_MAX_FILE_SIZE:
        max_bytes = settings.STUDENT_BULK_IMPORT_MAX_FILE_SIZE
...
def _serialize_preview_result(result):
    ...
    return StudentBulkImportPreviewResponseSerializer(payload).data
...
def _serialize_confirm_result(result):
    ...
    return StudentBulkImportConfirmResponseSerializer(payload).data
...
@extend_schema(
    ...
    description=(
        'Returns an Excel template for bulk student import. The workbook includes '
        'Import and Reference sheets plus dropdowns for gender, class, and guardian '
        'relationship values configured for the current school.'
    ),
    responses={
        200: OpenApiResponse(description='Excel template file (.xlsx).'),
    },
)
class StudentBulkImportTemplateView(SchoolScopedAPIView):
    def get(self, request):
        payload = build_student_import_template(self.school)
        return FileResponse(
            io.BytesIO(payload),
            as_attachment=True,
...
@extend_schema(
    tags=['Students'],
    summary='Upload students bulk import file',
    description=(
        'Uploads a student import file for preview or commit. Accepts .xlsx and .csv '
        ...
        'export when some rows fail. Guardian columns are optional.'
    ),
    parameters=[
        OpenApiParameter(
            name='dry_run',
    ...
    responses={
        200: StudentBulkImportPreviewResponseSerializer,
        201: StudentBulkImportConfirmResponseSerializer,
    },
)
class StudentBulkImportUploadView(SchoolScopedAPIView):
    ...
    def post(self, request):
        ...
        rows = parse_student_bulk_upload(file_bytes, upload_format=upload_format)

        if not rows:
            raise ValidationError({'file': 'No import rows were found in the uploaded file.'})

        if dry_run:
            result = preview_student_bulk_import(self.school, rows)
            return Response(_serialize_preview_result(result))

        result = commit_student_bulk_import(
            self.school,
            rows,
...
@extend_schema(
    tags=['Students'],
    summary='Download student bulk import failures',
    description=(
...
class StudentBulkImportFailuresDownloadView(SchoolScopedAPIView):
    def get(self, request, token):
        path, upload_format = load_failure_export(self.school.id, token)
        filename = f'student-import-failures.{upload_format}'
        content_type = (
